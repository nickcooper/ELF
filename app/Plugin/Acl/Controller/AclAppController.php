...
{
    var $components = array('Acl.AclManager', 'Acl.AclReflector');
	
    function beforeFilter()
    {
        ## II CUSTOM START ##
        $this->_fix_plugin_var();
        ## II CUSTOM END ##
    ...
    ## II CUSTOM START ##
    private function _fix_plugin_var()
    {
        $plugin = Configure::read('acl.model.plugin');

        if(preg_match('/^([^\.]+)(\.?)$/', $plugin, $m))
        {
            Configure::write('acl.model.plugin', sprintf('%s.', $m[1]));
        }
        //debug(Configure::read('acl.model.plugin'));
    }
    ## II CUSTOM END ##
	...
	{
	    $this->redirect($this->referer(array('action' => 'acos_index')));
	}
}
?>