...
class ArosController extends AclAppController
{
	...
	function beforeFilter()
	{
        ...
        $plugin = Configure :: read('acl.model.plugin');
        $this->loadModel(sprintf('%s%s', $plugin, Configure :: read('acl.aro.role.model')));
        $this->loadModel(sprintf('%s%s', $plugin, Configure :: read('acl.aro.user.model')));
        ## II CUSTOM END ##
	}
	...
	function users()
	{
	    ...
	    if(isset($this->request->data[Configure::read('acl.aro.user.model')][$user_display_field]) || $this->Session->check('acl.aros.users.filter'))
	    {
	        if(!isset($this->request->data[Configure::read('acl.aro.user.model')][$user_display_field]))
	        {
	            $this->request->data[Configure::read('acl.aro.user.model')][$user_display_field] = $this->Session->read('acl.aros.users.filter');
	        }
	        else
	        {
	            $this->Session->write('acl.aros.users.filter', $this->request->data[Configure::read('acl.aro.user.model')][$user_display_field]);
	        }
	        
	        $filter = array($user_model_name . '.' . $user_display_field . ' LIKE' => '%' . $this->request->data[Configure::read('acl.aro.user.model')][$user_display_field] . '%');
	    }
	    else
	    {
	        $filter = array();
	...
	function user_permissions($user_id = null)
	{
	    ...
	    if(empty($user_id))
	    {
    	    if(isset($this->request->data[Configure::read('acl.aro.user.model')][$user_display_field]) || $this->Session->check('acl.aros.user_permissions.filter'))
    	    {
    	        if(!isset($this->request->data[Configure::read('acl.aro.user.model')][$user_display_field]))
    	        {
    	            $this->request->data[Configure::read('acl.aro.user.model')][$user_display_field] = $this->Session->read('acl.aros.user_permissions.filter');
    	        }
    	        else
    	        {
    	            $this->Session->write('acl.aros.user_permissions.filter', $this->request->data[Configure::read('acl.aro.user.model')][$user_display_field]);
    	        }
	    ...
	    else
	    {
            ...
            else
            {
		    	 ...
		    	 */
		    	$count = $this->Aro->Permission->find('count', array('conditions' => array('Aro_id' => $user_aro[0]['Aro']['id'])));
		    	if($count != 0)
		    	{
	...
	function clear_user_specific_permissions($user_id)
	{
        ...
        if(empty($node))
        {
            ...
            $this->Session->setFlash(sprintf(__d('acl', "The %s '%s' does not exist in the ARO table", true), Configure::read('acl.aro.user.model'), $asked_user[Configure::read('acl.aro.user.model')][Configure :: read('acl.user.display_name')]), 'flash_error', null, 'plugin_acl');
        }
        else
        {
