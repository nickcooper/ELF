...
?>
    ...
    <h1><?php echo  __d('acl', Configure::read('acl.aro.user.model')) . ' : ' . $user[$user_model_name][$user_display_field]; ?></h1>
	...
	if($user_has_specific_permissions)
	{
    	...
    	echo ' (';
    	echo $this->Html->link($this->Html->image('/acl/img/design/cross2.png') . ' ' . __d('acl', 'Clear'), '/acl/aros/clear_user_specific_permissions/' . $user[$user_model_name][$user_pk_name], array('confirm' => __d('acl', 'Are you sure you want to clear the permissions specific to this user ?'), 'escape' => false));
    	echo ')';
    	echo '<div class="separator"></div>';
	}
	?>
