 ...
 * Loging test suite
 *
 * @package Logging.Test.Case.Behavior
...
class Article extends CakeTestModel
{
    public $name = 'Article';
    public $actsAs = array(
        'Logging.Auditable' => array('ignore' => array('ignored_field'))
    );
...
{
    public $hasMany = array('AuditDelta');
}

class AuditDelta extends CakeTestModel
{
    public $belongsTo = array('Audit');
}
    ...
    */
    public function testEdit()
    {
        ...
        $delta_records = $this->AuditDelta->find(
            ...
            'recursive' => -1,
            'conditions' => array(
                'AuditDelta.audit_id' => Set::extract('/Audit/id', $audit_records)
            ),
        )
        );
        ...
        $this->assertEqual('Second Test Article', 
            array_shift(
                Set::extract(
                    '/AuditDelta[property_name=title]/old_value', 
                    $last_audit
                )
            )
        );
        $this->assertEqual('Second Test Article (Newly Edited)',
            array_shift(
                Set::extract(
                    '/AuditDelta[property_name=title]/new_value',
                    $last_audit
                )
            )
        );

        $this->assertEqual('Second Test Article Body',
            array_shift(
                Set::extract(
                    '/AuditDelta[property_name=body]/old_value',
                    $last_audit
                )
            )
        );
        $this->assertEqual('Second Test Article Body (Also Edited)',
            array_shift(
                Set::extract(
                    '/AuditDelta[property_name=body]/new_value',
                    $last_audit
                )
            )
        );
        ...
        $this->assertEqual('N',
            array_shift(
                Set::extract(
                    '/AuditDelta[property_name=published]/old_value',
                    $last_audit
                )
            )
        );
        $this->assertEqual('Y',
            array_shift(
                Set::extract(
                    '/AuditDelta[property_name=published]/new_value',
                    $last_audit
                )
            )
        );
        ...
        $this->assertIdentical(array(),
            Set::extract(
                '/AuditDelta[property_name=ignored_field]',
                $last_audit
            )
        );
    }
