 ...
 *
 * @package Logging.Model.Behavior
...
class AuditableBehavior extends ModelBehavior
{
    ...
    public function setup(Model $Model, $settings=array())
    {
        if (! isset($this->settings[$Model->alias]))
        {
            $this->settings[$Model->alias] = array(
                'ignore' => array('created', 'updated', 'modified', 'password'),
        ...
        foreach ($this->settings[$Model->alias]['habtm'] as $index => $model_name )
        {
             ...
             */
            $hasAuditableBehavior = (boolean)(
                isset($Model->model_name)
                && is_array($Model->model_name->actsAs)
                && array_search('Auditable', $Model->$model_name->actsAs) == true
            );

            if (! array_key_exists($model_name, $Model->hasAndBelongsToMany) || $hasAuditableBehavior)
            {
    ...
    {
        $conditionKey = sprintf('%s.%s', $Model->alias, $Model->primaryKey);

        $original = $Model->find(
            'first', array('contain' => false, 'conditions' => array($conditionKey => $Model->id))
        );
    ...
    public function afterSave(Model $Model, $created)
    {
        ...
        $Model->Audit->bindModel(array('hasMany' => array('AuditDelta')));
        ...
        foreach ($audit[$Model->alias] as $property => $value)
        {
            ...
            if (! $created)
            {
                ...
                {
                     // If the property exists in the original _and_ the value is different, store it.
                     $delta = array(
                         'AuditDelta' => array(
        ...
        // Unset the original object vlaue so it's ready for the next call.
    ...
    private function _getModelData(Model $Model)
    {
         ...
         */
        $contain = array();
        if (! empty($this->settings[$Model->alias]['habtm']))
        {
            $contain = array_values($this->settings[$Model->alias]['habtm']);
        }
        
        $conditionKey = sprintf('%s.%s', $Model->alias, $Model->primaryKey);

        $data = $Model->find('first', array('contain' => $contain, 'conditions' => array($conditionKey => $Model->id)));
