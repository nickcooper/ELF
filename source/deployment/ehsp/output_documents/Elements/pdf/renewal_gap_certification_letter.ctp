...
if(isset($data['Application'][1]['expire_date']))
{ $license_old_expire_date = date("F j, Y", strtotime($data['Application'][1]['expire_date'])); }
else { $license_old_expire_date = date("F j, Y"); }

$license_effective_date = date("F j, Y", strtotime($data['Application'][0]['effective_date']));
...
<p>Your current certification will expire on {$license_expire_date}. Also, please be aware that you are required to take a refresher course by {$refresher_end_date}.</p>

<p>You were not Certified between {$license_old_expire_date} and {$license_effective_date}.</p>
